<?php
// filepath: c:\Users\fazlu\OneDrive\Desktop\projects\web_developmet\otp-service\send_otp.php
session_start();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    error_log("Generated OTP $otp for $username");

    if (storeOTP($username, $otp)) {
        error_log("OTP stored for $username, $otp");
        if (sendTelegramOTP($username, $otp)) {
            error_log("OTP sent via Telegram for $username");
            ...
            error_log("Failed to send OTP via Telegram for $username");
            echo json_encode([
                'success' => false, 
        ...
        error_log("Failed to store OTP for $username, OTP: $otp");
        echo json_encode([
            'success' => false, 
